-- ========================================
-- Fix Specific Trip Log: e21137bf-2147-4bd4-b788-ac256d62675c
-- แก้ไขทริปที่แสดง DT-2512-0020 ให้เป็น DT-2512-0022
-- ========================================
...
WHERE tl.delivery_trip_id = 'e21137bf-2147-4bd4-b788-ac256d62675c';

-- Step 2: หา delivery_trip ที่ถูกต้อง (DT-2512-0022)
SELECT 
  dt.id,
  dt.trip_number,
  dt.vehicle_id,
...
WHERE dt.trip_number = 'DT-2512-0022';
...
LEFT JOIN delivery_trips dt_correct ON 
  ...
  AND dt_correct.trip_number = 'DT-2512-0022'
WHERE tl.delivery_trip_id = 'e21137bf-2147-4bd4-b788-ac256d62675c';
...
BEGIN
  ...
  IF trip_log_record.id IS NULL THEN
    RAISE EXCEPTION 'ไม่พบ trip_log ที่มี delivery_trip_id = e21137bf-2147-4bd4-b788-ac256d62675c';
  END IF;
  ...
  WHERE delivery_trip_id = 'e21137bf-2147-4bd4-b788-ac256d62675c';
  
  RAISE NOTICE 'แก้ไขเสร็จสิ้น: เปลี่ยนจาก DT-2512-0020 เป็น DT-2512-0022';
...
WHERE dt.trip_number IN ('DT-2512-0020', 'DT-2512-0022')

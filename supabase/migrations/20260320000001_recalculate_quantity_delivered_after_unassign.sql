...
BEGIN
  ...
  -- ถ้ากำลังลบทริป completed ให้ตัดยอดที่เกิด "หลัง" เวลาในทริปนั้นออก
  -- เพื่อไม่ให้ยอดจาก completed ภายหลังไปถูกจัดสรรกลับมาให้ order ที่กำลัง unassign
  IF p_excluded_trip_id IS NOT NULL THEN
    SELECT dt.created_at
    INTO v_cutoff_ts
    FROM public.delivery_trips dt
    WHERE dt.id = p_excluded_trip_id;
  END IF;
  ...
  LOOP
    ...
    LOOP
      ...
      WHERE dt.status = 'completed'
        ...
        AND (v_cutoff_ts IS NULL OR dt.created_at <= v_cutoff_ts);
      ...
      LOOP
        ...
        LOOP
          EXIT WHEN v_remaining <= 0;
          -- ส่งให้ลูกค้าได้จริง = สั่งทั้งหมด - รับที่ร้านแล้ว
          v_assign := LEAST(
            v_remaining,
            GREATEST(0, COALESCE(v_oi.quantity, 0) - COALESCE(v_oi.picked_up_at_store, 0))
          );
